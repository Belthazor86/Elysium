...
if (isset($_GET['embedScript'])) {
    ...
    $allowed = realpath(__DIR__ . '/RomZilla');
...
if (isset($_GET['category'])) {
    ...
    $baseDir = __DIR__ . "/RomZilla/$category";
    $results = [];
            ...
            if ($query === '' || strpos($normalizedFolder, $query) !== false) {
                ...
                for ($i = 0; $i < $count; $i++) {
                    ...
                    $results[] = [
                        'title' => $folder,
                        'image' => $imageLink,
                        'script' => $scriptFile ? "Romzilla/$category/$folder/Scripts/" . basename($scriptFile) : null,
                    ];
                }
            }
        }

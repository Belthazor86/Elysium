...
function search_comics($query, $all = false) {
    ...
    foreach (glob($rootDir . '/*') as $folderPath) {
        ...
        // Scan the folder directly for images
        $files = array_filter(scandir($folderPath), function($f) {
            return preg_match('/\.(jpg|jpeg|png|webp|gif)$/i', $f);
        });

        // Natural sort so the first file is the cover
        sort($files, SORT_NATURAL);

        // Only add the first image (the cover) to search results
        if (!empty($files)) {
            $results[] = "$comicFolder/$folderName/" . $files[0];
        }
    }
    return $results;
}
...
<div class="search-box">
  <input type="text" id="searchInput" placeholder="Search..." autocomplete="off" oninput="doSearch()" />
  <!-- Search button removed – filtering happens automatically -->
...
function doSearch() {
  ...
  // If q is empty, we can either show nothing or show all? 
  // Our backend search_comics with empty q returns all folders (since stripos will match),
  // but we want to show all covers when the input is empty? 
  // Actually, with empty q, it will return all covers. That's fine.
  // But we can also decide to show all if empty. We'll keep as is: if empty, fetch with empty string.
...
function openSlideshow(imagePath) {
  // Extract the comic folder: "Graphique/ComicName"
  const parts = imagePath.split('/');
  const comicFolder = parts.slice(0, 2).join('/'); // e.g., "Graphique/Batman"
  
  fetch(`?ajax=images&folder=${encodeURIComponent(comicFolder)}`)
    .then(r => r.json())
    .then(images => {
      currentImages = images;
      // Start from the clicked image, if found
